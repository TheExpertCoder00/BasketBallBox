...
socket.addEventListener('message', (e) => {
  ...
  switch (data.type) {
    ...
    case 'score': {
      ...
      // show the between-points overlay AND lock inputs
      intermission = true;          // uses your flag from showIntermission()
      showIntermission();
      break;
    }

    // PATCH: unfreeze from server
    case 'resume': {
      intermission = false;
      const interEl = document.getElementById('intermission');
      if (interEl) interEl.style.display = 'none';

      // ensure we reflect the server ball on resume as well
      if (data.ball) {
        ball.position.set(data.ball.x, data.ball.y, data.ball.z);
        ballVelocity.set(data.ball.vx, data.ball.vy, data.ball.vz);
      }

      // place players per latest offense/defense (you already track lastRoles)
      if (lastRoles.offense) applySpawnForRoles(lastRoles.offense);
    ...
    case 'gameOver': {
      ...
      const finalMe = myRole === 'player1' ? data.scores.player1 : data.scores.player2;
      const finalThem = myRole === 'player1' ? data.scores.player2 : data.scores.player1;
