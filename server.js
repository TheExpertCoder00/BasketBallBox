...
wss.on('connection', (ws) => {
  ...
  ws.on('message', async (msg) => {
    ...
    // --- Auth (client-driven, no Firebase Admin required) ---
    if (data.type === 'auth') {
      if (data.uid) {
        ws._authState = 'confirmed';
        ws._user = { uid: data.uid, email: data.email || null, name: data.name || null };
      } else {
        ws._authState = 'unauthenticated';
        ws._user = null;
      }
      ws.send(JSON.stringify({ type: 'authAck', authState: ws._authState }));
      return;
    }
